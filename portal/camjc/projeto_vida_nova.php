...
$titulo       = 'Novo Projeto de Vida — ' . $a['nome'];
$pagina_ativa = 'camjc';
$erro = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!csrf_valido()) {
        ...
        $data_projeto = $_POST['data_projeto'] ?: date('Y-m-d');
        $metas        = trim($_POST['metas'] ?? '') ?: null;
        $nomes  = array_keys($todos_campos);
        $valores = array_map(fn($k) => trim($_POST[$k] ?? '') ?: null, $nomes);

        $sql = "INSERT INTO camjc_projetos_vida (acolhida_id, data_projeto, " . implode(', ', $nomes) . ", metas, criado_por)
                VALUES (?, ?, " . implode(',', array_fill(0, count($nomes), '?')) . ", ?, ?)";
        try {
            db()->prepare($sql)->execute([$acolhida_id, $data_projeto, ...$valores, $metas, $_SESSION['usuario_id'] ?? null]);
            $novo_id = (int)db()->lastInsertId();
            camjc_log('criou_projeto_vida', $acolhida_id, 'Projeto de Vida #' . $novo_id);
            header("Location: /portal/camjc/ver.php?id={$acolhida_id}&projeto_ok=1");
            exit;
        } catch (Exception $e) {
...
?>
<div class="form-wrap">
  <h2>Novo Projeto de Vida — <?= htmlspecialchars($a['nome']) ?></h2>
  <p style="font-size:.82rem;color:var(--muted);margin-top:-14px;margin-bottom:20px">Preenchido pela própria acolhida, com apoio da equipe. Pode ser refeito ao longo do tratamento.</p>
  ...
  <form method="post" novalidate>
    ...
    <div class="tab-pane ativo" data-tab-pane="auto">
      <div class="form-group">
        ...
        <input type="date" id="data_projeto" name="data_projeto" value="<?= htmlspecialchars($_POST['data_projeto'] ?? date('Y-m-d')) ?>">
      </div>
    ...
    <div class="tab-pane" data-tab-pane="metas">
      <div class="form-group">
        ...
        <span class="form-hint" style="display:block;margin-bottom:8px">Uma meta por linha, no formato: Meta — Estratégia — Prazo. Ex: "Perder 3kg — Caminhar 5x/semana, trocar lanches por frutas — 3 meses"</span>
        <textarea id="metas" name="metas" rows="10"><?= htmlspecialchars($_POST['metas'] ?? '') ?></textarea>
      </div>
    </div>
    ...
    <div class="form-acoes">
      ...
      <button type="submit" class="btn btn-primary">Salvar Projeto de Vida</button>
