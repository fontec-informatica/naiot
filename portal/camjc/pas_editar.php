...
$st = db()->prepare("SELECT * FROM camjc_pas WHERE id = ?");
$st->execute([$id]);
$p = $st->fetch();
if (!$p) { header('Location: /portal/camjc/'); exit; }

$st2 = db()->prepare("SELECT * FROM camjc_acolhidas WHERE id = ?");
$st2->execute([$p['acolhida_id']]);
$a = $st2->fetch();
if (!$a) { header('Location: /portal/camjc/'); exit; }

$titulo       = 'Editar Evolução / PAS — ' . $a['nome'];
$pagina_ativa = 'camjc';
$erro = '';
...
$atividades_atuais       = json_decode($p['atividades'] ?? '[]', true) ?: [];
$encaminhamentos_atuais  = json_decode($p['encaminhamentos'] ?? '[]', true) ?: [];

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!csrf_valido()) {
        $erro = 'Token inválido. Recarregue a página.';
    } else {
        $data_avaliacao = $_POST['data_avaliacao'] ?: $p['data_avaliacao'];
        ...
        try {
            db()->prepare("UPDATE camjc_pas SET data_avaliacao=?, $sets, atualizado_em=NOW() WHERE id=?")
                ->execute([$data_avaliacao, ...array_values($campos), $id]);
            camjc_log('editou_pas', $p['acolhida_id'], 'Evolução/PAS #' . $id);
            header("Location: /portal/camjc/ver.php?id={$p['acolhida_id']}&pas_editado=1");
            exit;
        } catch (Exception $e) {
...
$v = fn($campo) => htmlspecialchars($_POST[$campo] ?? $p[$campo] ?? '');
...
<div class="form-wrap">
  <h2>Editar Evolução Mensal / PAS — <?= htmlspecialchars($a['nome']) ?></h2>
  ...
  <form method="post" novalidate>
    ...
    <div class="tab-pane ativo" data-tab-pane="geral">
      <div class="form-group">
        ...
        <input type="date" id="data_avaliacao" name="data_avaliacao" value="<?= $v('data_avaliacao') ?: htmlspecialchars($p['data_avaliacao']) ?>">
      </div>
      <div class="form-group">
        <label>Houve alterações na medicação?</label>
        <div style="display:flex;gap:16px;margin-top:4px">
          <label style="display:flex;align-items:center;gap:6px;font-size:.85rem"><input type="radio" name="houve_alteracao_medicacao" value="sim" <?= (($_POST['houve_alteracao_medicacao'] ?? $p['houve_alteracao_medicacao']) === 'sim') ? 'checked' : '' ?>> Sim</label>
          <label style="display:flex;align-items:center;gap:6px;font-size:.85rem"><input type="radio" name="houve_alteracao_medicacao" value="nao" <?= (($_POST['houve_alteracao_medicacao'] ?? $p['houve_alteracao_medicacao'] ?? 'nao') === 'nao') ? 'checked' : '' ?>> Não</label>
        </div>
      </div>
      <div class="form-group">
        <label for="medicamentos">Medicamentos em uso</label>
        <textarea id="medicamentos" name="medicamentos" rows="5"><?= $v('medicamentos') ?></textarea>
      </div>
    </div>
    ...
    <div class="tab-pane" data-tab-pane="vinculo">
      <div class="form-group">
        ...
        <select id="vinculo_situacao" name="vinculo_situacao">
          ...
          <option value="<?= $chave ?>" <?= (($_POST['vinculo_situacao'] ?? $p['vinculo_situacao']) === $chave) ? 'selected' : '' ?>><?= htmlspecialchars($label) ?></option>
          <?php endforeach; ?>
        </select>
      </div>
      <div class="form-group">
        ...
        <select id="vinculo_qualidade" name="vinculo_qualidade">
          ...
          <option value="<?= $chave ?>" <?= (($_POST['vinculo_qualidade'] ?? $p['vinculo_qualidade']) === $chave) ? 'selected' : '' ?>><?= $label ?></option>
          <?php endforeach; ?>
        </select>
      </div>
      <div class="form-group">
        <label for="familiares_contato">Familiares com quem a acolhida manteve contato</label>
        <textarea id="familiares_contato" name="familiares_contato" rows="3"><?= $v('familiares_contato') ?></textarea>
      </div>
    </div>
    ...
    <div class="tab-pane" data-tab-pane="avaliacao">
      ...
      <div class="esc-grid">
        ...
        <select id="<?= $chave ?>" name="<?= $chave ?>">
          ...
          <option value="<?= $ec ?>" <?= (($_POST[$chave] ?? $p[$chave]) === $ec) ? 'selected' : '' ?>><?= $el ?></option>
          <?php endforeach; ?>
        </select>
      </div>
      <?php endforeach; ?>
    ...
    <div class="tab-pane" data-tab-pane="atividades">
      ...
      <?php foreach (CAMJC_ATIVIDADES_PAS as $chave => $label):
        $valor_atual = $_POST['ativ_' . $chave] ?? ($atividades_atuais[$chave] ?? '');
      ?>
      <div class="ativ-row">
        <span><?= htmlspecialchars($label) ?></span>
        <label><input type="radio" name="ativ_<?= $chave ?>" value="participa" <?= $valor_atual === 'participa' ? 'checked' : '' ?>> Participa</label>
        <label><input type="radio" name="ativ_<?= $chave ?>" value="dispensado" <?= $valor_atual === 'dispensado' ? 'checked' : '' ?>> Dispensado</label>
      </div>
      <?php endforeach; ?>
      ...
      <div class="encam-grid">
        <?php $encam_sel = $_POST['encam'] ?? $encaminhamentos_atuais; ?>
        <?php foreach (CAMJC_ENCAMINHAMENTOS_PAS as $chave => $label): ?>
        <label class="encam-check">
          <input type="checkbox" name="encam[]" value="<?= $chave ?>" <?= in_array($chave, $encam_sel, true) ? 'checked' : '' ?>>
      ...
      <div class="percepcao-block">
        <div class="form-group">
          ...
          <select id="percepcao_<?= $area ?>" name="percepcao_<?= $area ?>">
            <option value="">— Selecione —</option>
            <?php foreach (CAMJC_ESCALA_SATISFACAO as $ec => $el): ?>
            <option value="<?= $ec ?>" <?= (($_POST['percepcao_' . $area] ?? $p['percepcao_' . $area]) === $ec) ? 'selected' : '' ?>><?= $el ?></option>
            <?php endforeach; ?>
          </select>
        </div>
        <div class="form-group">
          <label for="percepcao_<?= $area ?>_melhorar">O que pode melhorar?</label>
          <input type="text" id="percepcao_<?= $area ?>_melhorar" name="percepcao_<?= $area ?>_melhorar" value="<?= $v('percepcao_' . $area . '_melhorar') ?>">
        </div>
      </div>
      <?php endforeach; ?>
    </div>
    ...
    <div class="tab-pane" data-tab-pane="parecer">
      <div class="form-group">
        <label>Quão importante é para ela fazer uma mudança sobre o uso de álcool/drogas agora? <span id="lbl_importancia" style="font-weight:700;color:var(--green-dk)"><?= htmlspecialchars($_POST['importancia_mudanca'] ?? $p['importancia_mudanca'] ?? '5') ?></span>/10</label>
        <div class="slider-wrap">
          <input type="range" name="importancia_mudanca" min="0" max="10" value="<?= htmlspecialchars($_POST['importancia_mudanca'] ?? $p['importancia_mudanca'] ?? '5') ?>" oninput="document.getElementById('lbl_importancia').textContent=this.value">
        </div>
      </div>
      <div class="form-group">
        <label>Quão confiante ela está de que pode ficar sem beber/usar substâncias? <span id="lbl_confianca" style="font-weight:700;color:var(--green-dk)"><?= htmlspecialchars($_POST['confianca_abstinencia'] ?? $p['confianca_abstinencia'] ?? '5') ?></span>/10</label>
        <div class="slider-wrap">
          <input type="range" name="confianca_abstinencia" min="0" max="10" value="<?= htmlspecialchars($_POST['confianca_abstinencia'] ?? $p['confianca_abstinencia'] ?? '5') ?>" oninput="document.getElementById('lbl_confianca').textContent=this.value">
        </div>
      </div>
      <div class="form-group">
        <label for="requerimentos_acolhida">Requerimentos e solicitações da acolhida</label>
        <textarea id="requerimentos_acolhida" name="requerimentos_acolhida" rows="3"><?= $v('requerimentos_acolhida') ?></textarea>
      </div>
      <div class="form-group">
        <label for="parecer_profissional">Parecer do profissional</label>
        <textarea id="parecer_profissional" name="parecer_profissional" rows="5"><?= $v('parecer_profissional') ?></textarea>
      </div>
      <div class="form-group">
        <label for="profissional_nome">Profissional responsável pelo preenchimento</label>
        <input type="text" id="profissional_nome" name="profissional_nome" value="<?= $v('profissional_nome') ?>">
      </div>
    </div>
    ...
    <div class="form-acoes">
      ...
      <a href="/portal/camjc/ver.php?id=<?= $p['acolhida_id'] ?>" class="btn btn-ghost" style="margin-left:auto">Cancelar</a>
    </div>
  </form>
</div>
