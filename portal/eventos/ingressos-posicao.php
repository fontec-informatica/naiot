...
$titulo       = 'Posição de Ingressos — ' . $evento['titulo'];
...
$sql = "
    ...
    WHERE evento_id = ? AND servo_nome IS NOT NULL
    GROUP BY servo_nome
...
if (isset($_GET['exportar'])) {
    ...
    header('Content-Disposition: attachment; filename="posicao_ingressos_evento' . $evento_id . '_' . date('Ymd') . '.csv"');
...
<div style="margin-bottom:16px">
  <h2 style="font-size:1rem;font-weight:600"><?= htmlspecialchars($evento['titulo']) ?> — Posição por servo</h2>
...
<div class="tabela-wrap">
  <div class="tabela-header" style="flex-wrap:wrap;gap:10px">
    ...
    <a href="?id=<?= $evento_id ?>&exportar=1" class="btn btn-ghost btn-sm" style="margin-left:auto">⬇ Exportar CSV</a>
  </div>
  ...
  <table>
    ...
    <tr>
      ...
      <td><a href="/portal/eventos/ingressos-acerto.php?id=<?= $evento_id ?>&servo=<?= urlencode($p['servo_nome']) ?>" class="btn btn-ghost btn-sm">Acertar</a></td>
    </tr>
    <?php endforeach; ?>
    </tbody>
    <tfoot>
