...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['acao'] ?? '') !== 'deletar') {
    if (!csrf_valido()) {
        ...
        if (!$nome_ev) {
            ...
            if (!$erro) {
                db()->prepare('UPDATE eventos SET titulo=?, descricao=?, data_evento=?, data_fim=?, local_evento=?, horario=?, imagem=?, ordem=?, ativo=? WHERE id=?')
                    ->execute([
                        $nome_ev,
                        $descricao ?: null,
                        $data_ev   ?: null,
  ...
  <form method="post" enctype="multipart/form-data" novalidate>
    ...
    <div class="form-group">
      <label style="display:flex;align-items:center;gap:8px;cursor:pointer">
        ...
        Evento ativo (visível no carrossel do site)
      </label>
    </div>
