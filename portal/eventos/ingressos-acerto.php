...
if (!$evento_id || $servo === '') { header('Location: /portal/eventos/'); exit; }
        ...
        if ($acao === 'liquidar') {
            ...
            if (!empty($_FILES['comprovante']['tmp_name']) && $_FILES['comprovante']['error'] === UPLOAD_ERR_OK) {
                ...
                if (isset($permitidos[$mime]) && $_FILES['comprovante']['size'] <= 10 * 1024 * 1024) {
                    ...
                    move_uploaded_file($_FILES['comprovante']['tmp_name'], __DIR__ . '/ingressos_uploads/' . $comprovante_nome);
                }
            }

            $sql = "UPDATE evento_ingressos SET status='pago', forma_pagamento=?, valor_pago=valor, data_pagamento=?, observacao=?"
                 . ($comprovante_nome ? ", comprovante=?" : "")
                 . " WHERE id IN ($ph) AND evento_id=? AND servo_nome=? AND status='distribuido'";
            $params = [$forma, $data_pag, $obs ?: null];
            if ($comprovante_nome) $params[] = $comprovante_nome;
            $params = array_merge($params, $ids, [$evento_id, $servo]);
            db()->prepare($sql)->execute($params);

        } elseif ($acao === 'devolver_lote') {
            db()->prepare("UPDATE evento_ingressos SET status='devolvido' WHERE id IN ($ph) AND evento_id=? AND servo_nome=? AND status='distribuido'")
                ->execute(array_merge($ids, [$evento_id, $servo]));
        }

        header("Location: /portal/eventos/ingressos-acerto.php?id=$evento_id&servo=" . urlencode($servo) . "&ok=1");
        exit;
    }
}

$lista = db()->prepare("SELECT * FROM evento_ingressos WHERE evento_id = ? AND servo_nome = ? ORDER BY numero ASC");
$lista->execute([$evento_id, $servo]);
$ingressos = $lista->fetchAll();

$total_distribuido = 0; $total_pago = 0; $total_pendente = 0;
...
<div style="margin-bottom:16px">
  ...
  <a href="/portal/eventos/ingressos.php?id=<?= $evento_id ?>" style="font-size:.82rem;color:var(--cinza3)">← Voltar aos ingressos</a>
</div>
    ...
    <tr>
      ...
      <td style="font-size:.8rem;color:var(--cinza3)">
        <?php if ($i['status'] === 'pago'): ?>
          ...
          <?php if ($i['comprovante']): ?><br><a href="/portal/eventos/ingressos_uploads/<?= htmlspecialchars($i['comprovante']) ?>" target="_blank">Ver comprovante</a><?php endif; ?>
        <?php endif; ?>
      </td>
    </tr>
    <?php endforeach; ?>
...
<?php if (!empty($total_pendente) || array_filter($ingressos, fn($i) => $i['status'] === 'distribuido')): ?>
