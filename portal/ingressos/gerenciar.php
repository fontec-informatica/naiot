    ...
    if ($acao === 'gerar_faixa') {
        ...
        if ($ini < 0 || $fim < $ini) {
            ...
            if ($chk->fetch()) {
                ...
                $pdo->commit();
                header("Location: /portal/ingressos/gerenciar.php?id=$campanha_id&ok=gerado");
                exit;
            }
        }
        ...
        if (!$servo_nome) {
            ...
            if (count($ids) < $quantidade) {
                ...
                header("Location: /portal/ingressos/gerenciar.php?id=$campanha_id&ok=distribuido");
                exit;
            }
        }
        ...
        if ($ini >= 0 && $fim >= $ini) {
            ...
            header("Location: /portal/ingressos/gerenciar.php?id=$campanha_id&ok=prep");
            exit;
        }
...
<div class="split-layout">
  ...
  <div>
    <div class="form-wrap" style="max-width:none;margin-bottom:20px">
      ...
      <form method="post" novalidate id="formGerar">
        <input type="hidden" name="csrf_token" value="<?= csrf_token() ?>">
