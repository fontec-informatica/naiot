...
<div style="display:flex;gap:10px;margin-bottom:16px;flex-wrap:wrap">
  <a href="/portal/estoque/categorias.php" class="btn btn-ghost btn-sm">Categorias</a>
  <a href="/portal/estoque/configuracoes.php" class="btn btn-ghost btn-sm">Configurações</a>
</div>
...
<div class="tabela-wrap">
  ...
  <form method="get" style="padding:14px 20px;border-bottom:1px solid var(--border);display:flex;gap:10px;flex-wrap:wrap;align-items:end">
    <div style="flex:1;min-width:180px">
      ...
      <input type="text" name="q" placeholder="Nome ou código" value="<?= htmlspecialchars($busca) ?>">
    </div>
    ...
    <tbody>
      ...
      <tr>
        <td>
          <?php if ($p['imagem']): ?>
            <img src="/assets/img/estoque/produtos/<?= htmlspecialchars($p['imagem']) ?>"
                 alt="" style="height:40px;width:40px;border-radius:6px;object-fit:cover">
          <?php else: ?>
            <div style="height:40px;width:40px;border-radius:6px;background:var(--off);border:1px solid var(--border)"></div>
          <?php endif; ?>
        </td>
        <td>
          <strong><?= htmlspecialchars($p['nome']) ?></strong>
          <br><small style="color:var(--cinza3)"><?= htmlspecialchars($p['codigo_barras'] ?: $p['codigo_interno']) ?></small>
        </td>
        <td>
        ...
        <td style="text-align:center">
          <?= (int)$p['estoque_atual'] ?> <?= htmlspecialchars($p['unidade']) ?>
